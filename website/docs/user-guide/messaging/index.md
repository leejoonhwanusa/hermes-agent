...
The Scheduled Task runs `wscript.exe` on a generated `.vbs` launcher under `%USERPROFILE%\.hermes\gateway-service\`. The launcher starts `python.exe -m hermes_cli.main gateway run` with a hidden window and **waits for that child to exit**. It then returns the child's exact exit code with `WScript.Quit`. This keeps the no-console protection at logon while letting Task Scheduler observe a gateway watchdog failure such as exit `75` (see `hermes_cli/gateway_windows.py::_build_gateway_vbs_script`).

:::info RestartOnFailure supervises the gateway through the launcher
The registered task keeps `wscript.exe` alive for the gateway lifetime, so a non-zero gateway exit is visible to Task Scheduler and its `<RestartOnFailure>` policy. The retry interval is one minute and the retry count is kept at `255`, the maximum value allowed by the Task Scheduler XML `unsignedByte` schema. `hermes gateway start` and `hermes gateway restart` use the registered task when it exists, preserving this supervisor chain. The Startup-folder fallback has no Task Scheduler restart policy and therefore does not provide the same crash-restart guarantee.
:::
